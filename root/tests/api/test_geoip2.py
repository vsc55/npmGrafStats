...
def geoip2_paths() -> dict[str, Path]:
    """
    Return paths to the GeoIP2 test DBs in tests/data.

    Used test/data/geoip2/GeoLite2-*-Test.mmdb. It does not matter if they are
    empty: in most tests Reader is mocked.
    """
    base = Path(__file__).resolve().parents[1] / "data"
    return {
        "city": str(base / "GeoLite2-City-Test.mmdb"),
        "asn": str(base / "GeoLite2-ASN-Test.mmdb"),
    }
...
def test_city_db_path_nonexistent_raises_path_error(monkeypatch, geoip2_paths):
    ...
    with pytest.raises(GeoIP2PathDBError) as exc:
        client.city_db_path = geoip2_paths["city"]

    assert "does not exist" in str(exc.value)
    assert exc.value.path == geoip2_paths["city"]


def test_city_db_path_existing_ok(monkeypatch, geoip2_paths):
    ...
    client.city_db_path = geoip2_paths["city"]
    assert client.city_db_path == geoip2_paths["city"]
...
def test_asn_db_path_nonexistent_raises_path_error(monkeypatch, geoip2_paths):
    ...
    with pytest.raises(GeoIP2PathDBError) as exc:
        client.asn_db_path = geoip2_paths["asn"]

    assert "does not exist" in str(exc.value)
    assert exc.value.path == geoip2_paths["asn"]


def test_asn_db_path_existing_ok(monkeypatch, geoip2_paths):
    ...
    client.asn_db_path = geoip2_paths["asn"]
    assert client.asn_db_path == geoip2_paths["asn"]
...
def test_read_db_invalid_database_raises_path_error(monkeypatch, geoip2_paths):
    """
    ...
    with pytest.raises(GeoIP2PathDBError) as exc:
        GeoIP2Client.read_db(geoip2_paths["city"])

    assert "not a valid MaxMind DB" in str(exc.value)
    assert exc.value.path == geoip2_paths["city"]


def test_city_without_ip_raises_config_error(monkeypatch, geoip2_paths):
...
def test_city_ok(monkeypatch, geoip2_paths):
...
def test_city_no_data_sets_status_no_data(monkeypatch, geoip2_paths):
...
def test_city_address_not_found_raises_config_error(monkeypatch, geoip2_paths):
...
def test_asn_without_ip_raises_config_error(monkeypatch, geoip2_paths):
...
def test_asn_ok(monkeypatch, geoip2_paths):
...
def test_asn_no_data(monkeypatch, geoip2_paths):
...
# ---------------------------------------------------------------------------
# Tests de métodos estáticos get_city / get_asn
# ---------------------------------------------------------------------------

def test_get_city_wrapper_uses_client(monkeypatch, geoip2_paths):
...
def test_get_asn_wrapper_uses_client(monkeypatch, geoip2_paths):
...
def test_debug_prints_city_result(monkeypatch, capsys, geoip2_paths):
